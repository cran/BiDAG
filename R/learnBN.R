...
#' @param algorithm MCMC scheme to be used for MAP structure learning; possible options are "order" (\code{\link{orderMCMC}}) or "orderIter" (\code{\link{iterativeMCMC}})
#' @param chainout logical, if TRUE the saved MCMC steps are returned, TRUE by default
#' @param scoreout logical, if TRUE the search space and score tables are returned; FALSE by default for "order", TRUE for "orderIter"
...
learnBN<-function(scorepar, algorithm = c("order", "orderIter"), chainout = FALSE,
                  scoreout = ifelse(algorithm=="orderIter",TRUE,FALSE), alpha = 0.05, moveprobs = NULL, iterations = NULL, stepsave = NULL, 
